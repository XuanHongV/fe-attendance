      ...
      `}>
        <div className="h-20 flex items-center justify-between px-6 border-b border-slate-800/50 bg-[#020617]/40">
          <div className="flex items-center gap-3 font-black text-white text-xl tracking-tighter">
            ...
            <span>PAYROLL<span className="text-blue-500">PRO</span></span>
          </div>
          <button 
            onClick={() => setMobileOpen(false)} 
        ...
        <div className="p-6">
          <div className="bg-slate-800/40 rounded-[1.5rem] p-4 flex items-center gap-4 border border-slate-700/50 shadow-inner">
            ...
            <div className="overflow-hidden">
              <p className="text-sm font-black text-white truncate tracking-tight">{user?.fullName || 'User Account'}</p>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                `}
              >
                <Icon className={`h-5 w-5 transition-transform duration-300 ${isActive ? 'scale-110' : 'text-slate-500 group-hover:scale-110'}`} />
