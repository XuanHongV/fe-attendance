...
import { WeeklyCalendarViewAdmin } from "./weekly-calendar-view-admin";
...
export const ScheduleManagement = () => {
  ...
  const [viewMode, setViewMode] = useState<"calendar" | "list" | "weekly">(
    "calendar"
  );
  ...
  const handleAssignSubmit = async () => {
    if (!selectedDateForAssign || !selectedEmployeeId || !selectedShiftId)
      return alert("Vui lòng chọn đầy đủ!");
    try {
      ...
      alert("Lỗi khi phân ca");
    }
  };

  const handleDeleteAssignment = async (id: string) => {
    // Truyền ít nhất 2 đối số: Tiêu đề và Nội dung
    const isConfirmed = await toastService.confirm(
      "Xác nhận hủy", // Tham số 1: title
      "Bạn có chắc chắn muốn hủy phân ca này không?", // Tham số 2: message
      "Đồng ý", // Tham số 3: okText (tùy chọn)
      "Bỏ qua" // Tham số 4: cancelText (tùy chọn)
    );
    ...
    try {
      ...
      toastService.success("Đã hủy phân ca thành công!");
  ...
  const renderCalendar = () => {
    ...
    for (let i = 0; i < firstDay; i++) {
      daysArray.push(
        <div
          key={`empty-${i}`}
          className="min-h-[120px] bg-gray-50/10 border-b border-r border-gray-100"
        ></div>
      );
    }

    for (let d = 1; d <= days; d++) {
      const dateString = `${year}-${(month + 1).toString().padStart(2, "0")}-${d
        .toString()
        .padStart(2, "0")}`;
      const dayAssignments = shifts.filter((s) =>
          ...
          <div className="space-y-1">
            {Object.entries(groupedByShift).map(([shiftName, assignments]) => (
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  setDetailModal({ isOpen: true, assignments });
                }}
                ...
                style={{
                  backgroundColor:
                    assignments[0].shift?.color_code || "#3b82f6",
              ...
              >
                ...
                <span className="bg-black/20 px-1 rounded text-[9px] font-black">
                  {assignments.length}
                </span>
              </button>
            ))}
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-8 font-sans">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-white p-6 rounded-[2rem] shadow-sm border border-slate-200/60">
          <div className="flex-1">
            <h1 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
              ...
              {isAdmin ? "Điều Hành Nhân Sự" : "Lịch Làm Việc Cá Nhân"}
            </h1>
        ...
        {loading ? (
          <div className="h-[400px] flex items-center justify-center bg-white rounded-[2.5rem] shadow-sm">
          ...
          <>
                ...
                <div className="overflow-x-auto">
                  <div className="min-w-[800px] grid grid-cols-7 border-t border-slate-100 font-black text-[10px] uppercase tracking-[0.2em] text-slate-400 bg-slate-50/30">
                    {["CN", "T2", "T3", "T4", "T5", "T6", "T7"].map((day) => (
                      <div
                        key={day}
                        className="py-4 text-center border-r border-slate-100 last:border-0"
                      >
                        {day}
                      </div>
                    ))}
                  </div>
                  <div className="min-w-[800px] grid grid-cols-7 border-t border-slate-100">
                    {renderCalendar()}
                  </div>
                </div>
              </div>
            ...
            {viewMode === "weekly" && (
              <WeeklyCalendarViewAdmin
                shifts={shifts}
                currentDate={currentDate}
                onDateChange={(newDate) => setCurrentDate(newDate)}
                onOpenDetail={(assignmentsInShift) => {
                  // Khi click vào Pill, mở modal danh sách nhân viên của ca đó
                  setDetailModal({
                    isOpen: true,
                    assignments: assignmentsInShift,
                  });
                }}
              />
            )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                {filteredShifts.map((item) => (
                  <div
                    ...
                    className="group bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 transition-all cursor-pointer relative overflow-hidden"
                  >
                    <div
                      className="absolute top-0 left-0 w-2 h-full"
                      style={{
                        backgroundColor: item.shift?.color_code,
                      }}
                    ></div>
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        ...
                        <h3 className="font-black text-slate-800 text-lg uppercase leading-tight">
                          {item.shift?.name}
                        </h3>
                      </div>
                    ...
                    <div className="pt-4 border-t border-slate-50 flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center text-white text-[10px] font-black uppercase">
          ...
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-[100] flex items-center justify-center p-4 animate-in zoom-in duration-200">
            <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden border border-slate-100">
              <div
                ...
                style={{
                  backgroundColor:
                    detailModal.assignments[0]?.shift?.color_code || "#3b82f6",
                }}
              >
                <button
                  onClick={() =>
                    setDetailModal({ isOpen: false, assignments: [] })
                  }
                  className="absolute top-1 right-2 p-2 hover:bg-white/20 rounded-full transition-colors"
                >
                  <X size={24} />
                </button>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-2xl font-black tracking-tight">
                      {detailModal.assignments[0]?.shift?.name}
                    </h3>
                    <div className="flex items-center gap-2 opacity-90 text-sm font-bold">
                      <Clock size={14} />
                      <span>
                        {detailModal.assignments[0]?.shift?.start_time} -{" "}
                        {detailModal.assignments[0]?.shift?.end_time}
                      </span>
                    </div>
                  </div>
                  <button
                    onClick={() =>
                      navigate(
                        `/shifts/qr/${detailModal.assignments[0]?.shift?._id}`
                      )
                    }
                    className="ml-auto p-2 bg-white/20 hover:bg-white/30 rounded-2xl backdrop-blur-md transition"
                    title="Mở mã QR điểm danh"
                  >
                    <QrCode size={28} />
                  </button>
                </div>
              ...
              <div className="p-8 bg-slate-50/50">
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-6 block">
                  Nhân sự thực hiện ({detailModal.assignments.length})
                </span>
                <div className="space-y-3 max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">
                  {detailModal.assignments.map((item, index) => (
                    <div
                      key={item._id}
                      onClick={() => setSelectedAssignment(item)}
                      className="group flex items-center justify-between p-4 bg-white rounded-3xl border border-gray-100 shadow-sm hover:border-blue-200 transition-all cursor-pointer animate-in slide-in-from-bottom"
                      style={{ animationDelay: `${index * 50}ms` }}
                    >
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-slate-100 flex items-center justify-center text-slate-600 font-black shadow-inner">
                          {item.user?.fullName?.charAt(0)}
                        </div>
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[110] flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-lg overflow-hidden border border-slate-100">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
                      ID: #{selectedAssignment._id.slice(-8).toUpperCase()}
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6">
                  <section>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 rounded-3xl bg-gradient-to-br from-blue-500 to-indigo-600 text-white flex items-center justify-center font-black text-xl shadow-lg">
                        {selectedAssignment.user?.fullName?.charAt(0)}
                      </div>
                      <div>
                        <h4 className="font-black text-slate-800 text-base">
                          {selectedAssignment.user?.fullName}
                        </h4>
                        <p className="text-xs text-slate-500 font-medium">
                          {selectedAssignment.user?.email || "N/A"}
                        </p>
                      </div>
                    </div>
                  </section>
              ...
              <div className="p-8 bg-slate-50/80 border-t border-slate-100 flex flex-col sm:flex-row gap-3 justify-between">
                <button
                  onClick={() => setSelectedAssignment(null)}
                  className="px-8 py-3.5 bg-white border border-slate-200 rounded-2xl text-slate-600 font-black hover:bg-slate-100 transition-all text-[11px] uppercase tracking-widest"
                >
                  Đóng
                </button>
                {isAdmin && (
                  <button
                    ...
                    className="flex items-center justify-center gap-2 px-6 py-3.5 bg-red-50 text-red-600 rounded-2xl font-black hover:bg-red-600 hover:text-white transition-all text-[11px] uppercase tracking-widest"
                  >
                    <Trash2 size={14} /> Hủy Phân Ca
                  </button>
                )}
              </div>
            </div>
            ...
            <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100">
              <div className="p-6 border-b border-slate-50 flex justify-between items-center bg-gray-50/50">
                ...
                <button
                  ...
                  className="p-2 bg-white rounded-full text-slate-400 hover:text-red-500 shadow-sm border border-slate-200"
                >
                  <X size={20} />
                </button>
              </div>
              <div className="p-6 space-y-5">
                <div className="space-y-2">
                  ...
                  <select
                    className="w-full bg-slate-50 border-none rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500/20 text-sm font-bold"
                ...
                <button
                  ...
                  className="w-full py-4 bg-blue-600 text-white rounded-[1.5rem] font-black shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all uppercase tracking-widest text-xs mt-2"
                >
                  Xác nhận
                </button>
              </div>
