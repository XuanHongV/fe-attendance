  ...
  switch (status) {
    case "PRESENT":
      return {
        icon: CheckCircle,
        color:
          "text-emerald-700 bg-emerald-100/80 border-emerald-200 shadow-emerald-100",
        text: "Có mặt",
      };
    case "LATE":
      return {
        icon: Clock,
        color:
          "text-amber-700 bg-amber-100/80 border-amber-200 shadow-amber-100",
        text: "Đi trễ",
      };
    case "DONE":
      return {
        icon: CheckCircle,
        color:
          "text-indigo-700 bg-indigo-100/80 border-indigo-200 shadow-indigo-100",
        text: "Hoàn thành",
      };
    case "ABSENT":
      return {
...
export const AttendanceManagement: React.FC = () => {
      ...
      const mappedAttendance: AttendanceRecord[] = attendanceRaw.map(
        (r: any) => {
          ...
          return {
            id: r._id || r.id,
            employeeName:
              r.employee?.fullName || r.employee?.name || "Nhân viên",
            avatar:
              r.employee?.avatar ||
  ...
  const filteredData = attendanceRecords.filter((r) => {
    ...
    // ⭐ CHỈ filter ngày khi KHÔNG ở chế độ "Tất cả"
    const matchDate = showAll ? true : r.date === selectedDate;
  ...
  const stats = {
    ...
    present: filteredData.filter(
      (r) => r.status === "DONE" || r.status === "PRESENT"
    ).length,
  ...
  return (
    <div className="p-5 bg-[#F4F7FA] min-h-screen font-sans text-slate-900">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <Users size={16} />
          {showAll ? "Chỉ hôm nay" : "Tất cả nhân viên"}
        </button>
